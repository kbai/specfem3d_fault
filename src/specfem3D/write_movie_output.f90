...
!================================================================
  ...
  if( ispec_is_elastic(ispec) ) then
     ! velocity x,y,z-components
      store_val_stressxx_external_mesh(narraydim*(ispec2D-1)+ipoin) = stress_xx_glob(iglob)
      store_val_stressyy_external_mesh(narraydim*(ispec2D-1)+ipoin) = stress_yy_glob(iglob)
      store_val_stresszz_external_mesh(narraydim*(ispec2D-1)+ipoin) = stress_zz_glob(iglob)
    ...
    do iface=1,num_free_surface_faces
       ...
       if (USE_HIGHRES_FOR_MOVIES) then
          do igll = 1, NGLLSQUARE
             ...
             k = free_surface_ijk(3,igll,iface)
             iglob = ibool(i,j,k,ispec)
!             write(*,*) 'put displ into storage array!'
            ! puts displ/velocity values into storage array
   ...
   ! master collects wavefield
    if( myrank == 0 ) then
       ...
       call gatherv_all_cr(store_val_uz_external_mesh,nfaces_surface_ext_mesh_points,&
            store_val_uz_all_external_mesh,nfaces_perproc_surface_ext_mesh,faces_surface_offset_ext_mesh,&
            nfaces_surface_glob_em_points,NPROC)
        call gatherv_all_cr(store_val_stressxx_external_mesh,nfaces_surface_ext_mesh_points,&
            store_val_stressxx_all_external_mesh,nfaces_perproc_surface_ext_mesh,faces_surface_offset_ext_mesh,&
            nfaces_surface_glob_em_points,NPROC)
       call gatherv_all_cr(store_val_stressyy_external_mesh,nfaces_surface_ext_mesh_points,&
            store_val_stressyy_all_external_mesh,nfaces_perproc_surface_ext_mesh,faces_surface_offset_ext_mesh,&
            nfaces_surface_glob_em_points,NPROC)
       call gatherv_all_cr(store_val_stresszz_external_mesh,nfaces_surface_ext_mesh_points,&
            store_val_stresszz_all_external_mesh,nfaces_perproc_surface_ext_mesh,faces_surface_offset_ext_mesh,&
            nfaces_surface_glob_em_points,NPROC)
       call gatherv_all_cr(store_val_stressxy_external_mesh,nfaces_surface_ext_mesh_points,&
            store_val_stressxy_all_external_mesh,nfaces_perproc_surface_ext_mesh,faces_surface_offset_ext_mesh,&
            nfaces_surface_glob_em_points,NPROC)
       call gatherv_all_cr(store_val_stressxz_external_mesh,nfaces_surface_ext_mesh_points,&
            store_val_stressxz_all_external_mesh,nfaces_perproc_surface_ext_mesh,faces_surface_offset_ext_mesh,&
            nfaces_surface_glob_em_points,NPROC)
    ...
    if(myrank == 0) then
       ...
       write(IOUT) store_val_uy_all_external_mesh  ! velocity y-component
       write(IOUT) store_val_uz_all_external_mesh  ! velocity z-component
       write(IOUT) store_val_stressxx_all_external_mesh !
...
! outputs movie files for div, curl and velocity
  ...
  !
    ...
    open(unit=27,file=trim(OUTPUT_FILES)//trim(outputname),status='unknown',form='unformatted',iostat=ier)
    if( ier /= 0 ) stop 'error opening file stress_yy_glob'
    write(27) stress_yy
    close(27)

    write(outputname,"('/proc',i6.6,'_stress_xy_it',i6.6,'.bin')") myrank,it
    open(unit=27,file=trim(OUTPUT_FILES)//trim(outputname),status='unknown',form='unformatted',iostat=ier)
    if( ier /= 0 ) stop 'error opening file stress_xy_glob'
    write(27) stress_xy
    close(27)
...
!  curl_glob(:) = 0.0_CUSTOM_REAL
!  valency(:) = 0
   stress_xx_glob(:) = 0.0_CUSTOM_REAL
   stress_yy_glob(:) = 0.0_CUSTOM_REAL
...
!write(*,*) 'VALUE VALENCY GIVEN SUCCESSFULLY!'
  ...
  do ispec=1,NSPEC_AB
    ...
    do k=1,NGLLZ
      ...
      enddo
    enddo
!      write(*,*) 'partial derivative has been calculated!'
    do k = 1,NGLLZ
      do j = 1,NGLLY
        do i = 1,NGLLX
...
!      curl_glob(i) = curl_glob(i)/valency(i)
  ...
  end subroutine wmo_movie_stress

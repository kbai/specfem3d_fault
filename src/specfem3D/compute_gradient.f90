...
! double loop over GLL points to compute and store gradients
  ...
  subroutine compute_gradient_norho(ispec,NSPEC_AB,NGLOB_AB, &
                        scalar_field, vector_field_element,&
                        hprime_xx,hprime_yy,hprime_zz, &
                        xix,xiy,xiz,etax,etay,etaz,gammax,gammay,gammaz, &
                        ibool)
...
! double loop over GLL points to compute and store gradients
 ...
 !       endif
        ...
        ! derivatives of acoustic scalar potential field on GLL points
        vector_field_element(1,i,j,k) = (temp1l*xixl + temp2l*etaxl + temp3l*gammaxl) 
        vector_field_element(2,i,j,k) = (temp1l*xiyl + temp2l*etayl + temp3l*gammayl) 
        vector_field_element(3,i,j,k) = (temp1l*xizl + temp2l*etazl + temp3l*gammazl) 

      enddo
    enddo
  enddo

  end subroutine compute_gradient_norho
